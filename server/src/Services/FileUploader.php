...
class FileUploader
{
   ...
   public function uploadFile(UploadedFile $file, string $fileName = ''): string
   {
      ...
      try {
         ...
         throw $this->createServiceException("Cannot move file to directory");
      }
    ...
    */
   private function createTargetDirectoryIfNotExist()
   {
      if (!$this->fileSystem->exists($this->targetDirectory)) {
         try {
            $this->fileSystem->mkdir($this->targetDirectory);
         } catch (IOException  $exception) {
            throw $this->createServiceException("It's impossible upload solution right now, try later");
         }
      }
   }
    ...
    */
   private function createServiceException(string $message): FileUploaderException
   {
